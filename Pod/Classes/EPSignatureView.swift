...
open class EPSignatureView: UIView {
    ...
    open var signatureGestureRecognizer:UIPanGestureRecognizer?
    ...
    private func addLongPressGesture() {
        ...
        let panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(EPSignatureView.panGestureRecognized(_:)))
    ...
    @objc func panGestureRecognized(_ gesture: UIPanGestureRecognizer) {
        ...
        case .began:
            ...
            isSigned = true
            bezierPoints[0] = gesture.location(in: self)
            bezierCounter = 0
            setNeedsDisplay()
        case .changed:
